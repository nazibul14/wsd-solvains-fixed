...
build_dev() {
  ...
  $docker_compose_cmd exec -T "$php_fpm_container_name" composer update
  # Fix log file permissions
  $docker_compose_cmd exec -T "$php_fpm_container_name" mkdir -p /project/data/log
  $docker_compose_cmd exec -T "$php_fpm_container_name" touch /project/data/log/application.log
  $docker_compose_cmd exec -T "$php_fpm_container_name" chmod 777 /project/data/log/application.log
  $docker_compose_cmd exec -T "$php_fpm_container_name" chmod 777 /project/data/log
  set +x
  wait
}
